...
namespace Redemption.Projectiles.Minions
{
    public class HoloMinion : ModProjectile
    {
                ...
                case swordMode:
                    if (attackCooldown < 0)
                    {
                        if (RedeHelper.ClosestNPC(ref target, 400, Projectile.Center, true, player.MinionAttackTargetNPC))
                        {
                            ...
                            attackCooldown = 20;
                            if (Projectile.Center.Distance(player.Center) > 800)
                                mode = chickenMode;
                        }
                        else
                        {
        ...
        private bool CheckActive(Player owner)
        {
            ...
            {
                owner.ClearBuff(BuffType<HoloMinionBuff>());

                return false;
            }

            if (owner.HasBuff(BuffType<HoloMinionBuff>()))
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {
                if (mode == eyeMode)
                    texture = Request<Texture2D>(Texture + "3").Value;
                else
                    texture = Request<Texture2D>(Texture + "2").Value;
            }
