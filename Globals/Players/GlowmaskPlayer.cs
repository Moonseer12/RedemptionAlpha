...
{
    // Code from Spirit Mod
    public class RedeGlowmask : ModPlayer
    {
        internal static readonly Dictionary<int, Texture2D> ItemGlowMask = new();

        internal new static void Unload() => ItemGlowMask.Clear();
        public static void AddGlowMask(int itemType, string texturePath) => ItemGlowMask[itemType] = Request<Texture2D>(texturePath, AssetRequestMode.ImmediateLoad).Value;
    }

    public class RedeGlowMaskItemLayer : PlayerDrawLayer
    {
        ...
        protected override void Draw(ref PlayerDrawSet drawInfo)
        {
            ...
            if (item.type >= ItemID.Count && RedeGlowmask.ItemGlowMask.TryGetValue(item.type, out Texture2D textureItem) && (drawInfo.drawPlayer.itemTime > 0 || item.useStyle != ItemUseStyleID.None)) //Held ItemType
                GlowmaskPlayer.DrawItemGlowMask(textureItem, drawInfo);
        }
    }

    public class RedeGlowMaskLegsLayer : PlayerDrawLayer
    {
        public override Position GetDefaultPosition() => new AfterParent(PlayerDrawLayers.Leggings);

        protected override void Draw(ref PlayerDrawSet drawInfo)
        {
            if (drawInfo.drawPlayer.armor[12].type >= ItemID.Count && RedeGlowmask.ItemGlowMask.TryGetValue(drawInfo.drawPlayer.armor[12].type, out Texture2D vanity)) //Vanity Legs
                GlowmaskPlayer.DrawArmorGlowMask(GlowmaskPlayer.ArmorContext.Legs, vanity, drawInfo);
            else if (drawInfo.drawPlayer.armor[2].type >= ItemID.Count && RedeGlowmask.ItemGlowMask.TryGetValue(drawInfo.drawPlayer.armor[2].type, out Texture2D armor))
                GlowmaskPlayer.DrawArmorGlowMask(GlowmaskPlayer.ArmorContext.Legs, armor, drawInfo);
        }
    }

    public class RedeGlowMaskBodyLayer : PlayerDrawLayer
    {
        public override Position GetDefaultPosition() => new AfterParent(PlayerDrawLayers.Torso);

        protected override void Draw(ref PlayerDrawSet drawInfo)
        {
            if (drawInfo.drawPlayer.armor[11].type >= ItemID.Count && RedeGlowmask.ItemGlowMask.TryGetValue(drawInfo.drawPlayer.armor[11].type, out Texture2D vanity)) //Vanity Body
                GlowmaskPlayer.DrawArmorGlowMask(GlowmaskPlayer.ArmorContext.Body, vanity, drawInfo);
            else if (drawInfo.drawPlayer.armor[1].type >= ItemID.Count && RedeGlowmask.ItemGlowMask.TryGetValue(drawInfo.drawPlayer.armor[1].type, out Texture2D armor))
                GlowmaskPlayer.DrawArmorGlowMask(GlowmaskPlayer.ArmorContext.Body, armor, drawInfo);
        }
    }

    public class RedeGlowMaskArmsLayer : PlayerDrawLayer
    {
        ...
        {
            if (drawInfo.drawPlayer.armor[11].type >= ItemID.Count && RedeGlowmask.ItemGlowMask.TryGetValue(drawInfo.drawPlayer.armor[11].type, out Texture2D vanity)) //Vanity Body
                GlowmaskPlayer.DrawArmorGlowMask(GlowmaskPlayer.ArmorContext.Arms, vanity, drawInfo);
            else if (drawInfo.drawPlayer.armor[1].type >= ItemID.Count && RedeGlowmask.ItemGlowMask.TryGetValue(drawInfo.drawPlayer.armor[1].type, out Texture2D armor))
                GlowmaskPlayer.DrawArmorGlowMask(GlowmaskPlayer.ArmorContext.Arms, armor, drawInfo);
        }
    }

    public class RedeGlowMaskHeadLayer : PlayerDrawLayer
    {
        public override Position GetDefaultPosition() => new AfterParent(PlayerDrawLayers.Head);

        protected override void Draw(ref PlayerDrawSet drawInfo)
        {
            if (drawInfo.drawPlayer.armor[10].type >= ItemID.Count && RedeGlowmask.ItemGlowMask.TryGetValue(drawInfo.drawPlayer.armor[10].type, out Texture2D vanity)) //Vanity Head
                GlowmaskPlayer.DrawArmorGlowMask(GlowmaskPlayer.ArmorContext.Head, vanity, drawInfo);
            else if (drawInfo.drawPlayer.armor[0].type >= ItemID.Count && RedeGlowmask.ItemGlowMask.TryGetValue(drawInfo.drawPlayer.armor[0].type, out Texture2D armor))
                GlowmaskPlayer.DrawArmorGlowMask(GlowmaskPlayer.ArmorContext.Head, armor, drawInfo);
        }
    }
    public static class GlowmaskPlayer
    {
                ...
                case ArmorContext.Head:
                    {
                        ...
                        DrawData drawData = new(texture, pos, info.drawPlayer.bodyFrame, info.headGlowColor, info.drawPlayer.headRotation, info.rotationOrigin, 1f, info.playerEffect, 0)
                        { shader = info.cHead };
                ...
                case ArmorContext.Body:
                    {
                        ...
                        {
                            DrawData drawData = new(texture, pos + bobOff, info.compTorsoFrame, info.bodyGlowColor, info.drawPlayer.bodyRotation, info.rotationOrigin, 1f, info.playerEffect)
                            { shader = info.cBody };
                        ...
                        {
                            DrawData drawData = new(texture, pos + bobOff, info.drawPlayer.bodyFrame, info.bodyGlowColor, info.drawPlayer.bodyRotation, info.rotationOrigin, 1f, info.playerEffect, 0)
                            { shader = info.cBody };
                ...
                case ArmorContext.Arms:
                    {
                        ...
                        if (info.usesCompositeTorso)
                        {
                            ...
                            DrawData drawData = new(texture, pos + bobOff, info.compFrontArmFrame, info.bodyGlowColor, rotation, bodyVect, 1f, info.playerEffect)
                            { shader = info.cBody };
                            ...
                            {
                                DrawData drawData2 = new(texture, shoulderPos + bobOff, info.compFrontShoulderFrame, info.bodyGlowColor, info.drawPlayer.bodyRotation, bodyVect, 1f, info.playerEffect)
                                { shader = info.cBody };
                ...
                case ArmorContext.Legs:
                    {
                        ...
                        if (info.drawPlayer.shoe != 15 || info.drawPlayer.wearsRobe)
                        {
                            ...
                            DrawData drawData = new(texture, pos, info.drawPlayer.legFrame, info.legsGlowColor, info.drawPlayer.legRotation, info.rotationOrigin, 1f, info.playerEffect, 0)
                            { shader = info.cLegs };
