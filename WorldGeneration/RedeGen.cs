...
namespace Redemption.WorldGeneration
{
    public class RedeGen : ModSystem
    {
        ...
        {
            /*
            bool buddingBolinePlaced = false;
            for (int chestIndex = 0; chestIndex < 1000; chestIndex++)
            {
                ...
                {
                    if (!buddingBolinePlaced || Main.rand.NextBool(3))
                    {
            ...
            for (int chestIndex = 0; chestIndex < Main.maxChests; chestIndex++)
            {
                ...
                {
                    if (!seaEmblemPlaced || WorldGen.genRand.NextBool(5))
                    {
            ...
            while (placed1 && placed2 < 30 && attempts++ < 100000)
            {
                ...
                switch (WorldGen.genRand.Next(2))
                {
                    case 0:
                        ...
                        WorldGen.PlaceObject(placeX3, placeY - 1, ModContent.TileType<ThornsTile>(), true, rand);
                        break;
                    case 1:
                        rand = WorldGen.genRand.Next(2);
                        WorldGen.PlaceObject(placeX3, placeY - 1, ModContent.TileType<ThornsTile2>(), true, rand);
                        break;
                }
                placed2++;
            }
        ...
        public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight)
        {
            ...
            if (ShiniesIndex != -1)
            {
                tasks.Insert(ShiniesIndex + 2, new PassLegacy("P L A N T", delegate (GenerationProgress progress, GameConfiguration configuration)
                {
                ...
                tasks.Insert(ShiniesIndex + 3, new PassLegacy("Infested Stone", delegate (GenerationProgress progress, GameConfiguration configuration)
                {
                ...
                tasks.Insert(ShiniesIndex + 4, new PassLegacy("Dragon Fossils", delegate (GenerationProgress progress, GameConfiguration configuration)
                {
                    #region Dragon-Lead
                ...
                tasks.Insert(ShiniesIndex + 6, new PassLegacy("Icy Ancient Decal", delegate (GenerationProgress progress, GameConfiguration configuration)
                {
            ...
            if (ShiniesIndex2 != -1)
            {
                if (CrossMod.CrossMod.NewBeginnings.Enabled)
                {
                    if ((bool)CrossMod.CrossMod.NewBeginnings.Instance.Call("PlayerHasOrigin", Main.LocalPlayer, "MoRSpiritWalker"))
                    {
                        tasks.Insert(ShiniesIndex2 + 1, new PassLegacy("Portals 2", delegate (GenerationProgress progress, GameConfiguration configuration)
                        {
                            UndergroundPortalGen(ref progress);
                        }));
                    }
                    if ((bool)CrossMod.CrossMod.NewBeginnings.Instance.Call("PlayerHasOrigin", Main.LocalPlayer, "MoRSkySquire"))
                    {
                        tasks.Insert(ShiniesIndex2 + 1, new PassLegacy("Jo Shrine", delegate (GenerationProgress progress, GameConfiguration configuration)
                        {
                ...
                tasks.Add(new PassLegacy("Special  Ancient Decal", delegate (GenerationProgress progress, GameConfiguration configuration)
                {
                    #region Ancient Decal Special
                    progress.Message = "Putting spirits to rest..Assassin's Tomb";
                    bool placed = false;
                    int attempts = 0;
                    while (!placed && attempts++ < 10000)
                    {
                        if (attempts > 500)
                            progress.Message = "Putting spirits to rest..Assassin's Tomb\n(Attempts to find valid spot: " + attempts + "/2000)";
                        if (attempts > 2000)
                            progress.Message = "Putting spirits to rest..Assassin's Tomb\n(Failsafe: Ignoring other biome tiles)";

                        int tilesX = WorldGen.genRand.Next((int)(Main.maxTilesX * .3f), (int)(Main.maxTilesX * .7f));
                        int tilesY = WorldGen.genRand.Next((int)(Main.maxTilesY * .4f), (int)(Main.maxTilesY * .7f));
                        if (!WorldGen.InWorld(tilesX, tilesY))
                            continue;
                        ...
                        for (int x = 0; x < 25 + (25 * roomNum); x++)
                        {
                            for (int y = 0; y < 39; y++)
                            {
                                bool a = !GenVars.structures.CanPlace(new Rectangle(tilesX, tilesY, x, y));
                                if (Main.remixWorld || Main.zenithWorld)
                                    a = false;

                                int type = Framing.GetTileSafely(tilesX + x, tilesY + y).TileType;
                                if (!WorldGen.InWorld(tilesX + x, tilesY + y) || TileLists.BlacklistTiles.Contains(type) || TileID.Sets.Conversion.Sandstone[type] || a)
                                {
                                    blacklist = true;
                                    break;
                                }
                                bool onTile = type is TileID.Stone or TileID.Dirt;
                                if (Main.remixWorld || Main.zenithWorld)
                                    onTile = type is TileID.Stone or TileID.Dirt or TileID.Ebonstone or TileID.Crimstone;
                                if (Framing.GetTileSafely(tilesX + x, tilesY + y).HasTile && onTile)
                                    stoneScore++;
                                else if (!TileID.Sets.Ore[type])
                                    emptyScore++;
                            }
                        ...
                        if (blacklist)
                            continue;
                        if (stoneScore < (emptyScore * 1.5f) && attempts < 2000)
                            continue;

                        Vector2 origin = new(tilesX, tilesY);
                        StructureHelper.API.Legacy.LegacyGenerator.GenerateStructure("WorldGeneration/AncientDecalRSpirit1", origin.ToPoint16(), Mod);
                        Point16 dims = Point16.Zero;
                        StructureHelper.API.Legacy.LegacyGenerator.GetDimensions("WorldGeneration/AncientDecalRSpirit1", Mod, ref dims);
                        GenVars.structures.AddProtectedStructure(new Rectangle((int)origin.X, (int)origin.Y, dims.X, dims.Y));
                        ...
                        for (int x = 0; x < 25 + (25 * roomNum); x++)
                        {
                            for (int y = 0; y < 39; y++)
                            {
                                if (WorldGen.InWorld(tilesX + x, tilesY + y))
                                {
                                    ...
                                    {
                                        if (WorldGen.genRand.NextBool(8))
                                            GenUtils.PlaceSkeletalRemains(tilesX + x, tilesY + y);
                                    }
                                    if (Framing.GetTileSafely(tilesX + x, tilesY + y).TileType == ModContent.TileType<ElderWoodPlatformTile>())
                                        WorldGen.KillTile(tilesX + x, tilesY + y, true);
                                    if (WorldGen.genRand.NextBool(5) && Framing.GetTileSafely(tilesX + x, tilesY + y).TileType == ModContent.TileType<AncientGoldCoinPileTile>() && Framing.GetTileSafely(tilesX + x, tilesY + y - 1).TileType != ModContent.TileType<AncientGoldCoinPileTile>())
                    ...
                    progress.Message = "Putting spirits to rest..Old Man's Tomb";
                    progress.Set(.2f);

                    bool placed3 = false;
                    attempts = 0;
                    while (!placed3 && attempts++ < 10000)
                    {
                        if (attempts > 500)
                            progress.Message = "Putting spirits to rest..Old Man's Tomb\n(Attempts to find valid spot: " + attempts + "/2000)";
                        if (attempts > 2000)
                        ...
                        Point16 dims = Point16.Zero;
                        StructureHelper.API.Legacy.LegacyGenerator.GetDimensions("WorldGeneration/AncientDecalLSpirit1", Mod, ref dims);
                        GenVars.structures.AddProtectedStructure(new Rectangle((int)origin.X, (int)origin.Y, dims.X, dims.Y));
                    ...
                    progress.Message = "Putting spirits to rest..Frozen Tomb";
                    progress.Set(.4f);

                    bool placed4 = false;
                    attempts = 0;
                    while (!placed4 && attempts++ < 10000)
                    {
                        if (attempts > 500)
                            progress.Message = "Putting spirits to rest..Frozen Tomb\n(Attempts to find valid spot: " + attempts + "/2000)";
                        if (attempts > 2000)
                        ...
                        for (int x = 0; x < 30 + (30 * roomNum); x++)
                        {
                            for (int y = 0; y < 50; y++)
                            {
                                ...
                                int type = Framing.GetTileSafely(tilesX + x, tilesY + y).TileType;
                                if (!WorldGen.InWorld(tilesX + x, tilesY + y) || TileLists.BlacklistTiles.Contains(type) || a)
                                {
                                    blacklist = true;
                                    break;
                                }
                        ...
                        for (int i = 0; i < roomNum; i++)
                        {
                            ...
                            if (i == bigRoom)
                            {
                                ...
                                Point16 dims = Point16.Zero;
                                StructureHelper.API.Legacy.LegacyGenerator.GetDimensions("WorldGeneration/IceDecalMSpecial1", Mod, ref dims);
                                GenVars.structures.AddProtectedStructure(new Rectangle((int)origin.X, (int)origin.Y, dims.X, dims.Y));
                                origin.X += 30;
                                origin.Y += 6;
                            }
                    ...
                    progress.Message = "Putting spirits to rest..Knight's Tomb";
                    progress.Set(.6f);

                    bool placed2 = false;
                    attempts = 0;
                    while (!placed2 && attempts++ < 10000)
                    {
                        if (attempts > 500)
                            progress.Message = "Putting spirits to rest..Knight's Tomb\n(Attempts to find valid spot: " + attempts + "/2000)";
                        if (attempts > 2000)
                        ...
                        if (blacklist)
                            continue;
                        if (stoneScore < (int)(emptyScore * 1.5) && attempts < 2000)
                            continue;

                        Vector2 origin = new(tilesX, tilesY);
                        StructureHelper.API.Legacy.LegacyGenerator.GenerateStructure("WorldGeneration/CommonGuardTomb", origin.ToPoint16(), Mod);
                        Point16 dims = Point16.Zero;
                        StructureHelper.API.Legacy.LegacyGenerator.GetDimensions("WorldGeneration/CommonGuardTomb", Mod, ref dims);
                        GenVars.structures.AddProtectedStructure(new Rectangle((int)origin.X, (int)origin.Y, dims.X, dims.Y));
                        for (int x = 0; x < 53; x++)
                        {
                            for (int y = 0; y < 33; y++)
                    ...
                    progress.Message = "Putting spirits to rest..Jungle Tomb";
                    progress.Set(.8f);

                    bool placed5 = false;
                    attempts = 0;
                    while (!placed5 && attempts++ < 10000)
                    {
                        if (attempts > 500)
                            progress.Message = "Putting spirits to rest..Jungle Tomb\n(Attempts to find valid spot: " + attempts + "/2000)";
                        if (attempts > 2000)
                ...
                tasks.Add(new PassLegacy("Portals", delegate (GenerationProgress progress, GameConfiguration configuration)
                {
                    ...
                    while (!placed)
                    {
                        ...
                        if (attempts2 < Main.maxTilesX / 2)
                            progress.Message = "Thinking with portals...Finding Flat Ground (" + attempts2 + "/" + Main.maxTilesX / 2 + ")";
                        else
                            progress.Message = "Thinking with portals...Finding literally anywhere to place this darn portal";
                        ...
                        for (int m = -1; m <= 1; m += 2)
                        {
                            ...
                            if (Main.remixWorld || Main.zenithWorld)
                                placeY = WorldGen.genRand.Next((int)Main.worldSurface - 160, (int)(Main.maxTilesY * .9f));
                            ...
                            if (onTile)
                                continue;
                            if (attempts2 < (Main.maxTilesX / 2) && !CheckFlat(placeX, placeY, 10, 2))
                                continue;

                            origin = new(placeX - 34, placeY - 11);
                            if (attempts2 < Main.maxTilesX && !GenVars.structures.CanPlace(new Rectangle((int)origin.X, (int)origin.Y, 60, 82)))
                                continue;
                    ...
                    for (int i = originPoint.X; i < originPoint.X + 60; i++)
                    {
                        for (int j = originPoint.Y; j < originPoint.Y + 82; j++)
                        {
                                ...
                                Framing.GetTileSafely(i, j).HasTile)
                            {
                                ...
                                {
                                    int rand = WorldGen.genRand.Next(47, 49);
                                    WorldGen.PlaceObject(i, j - 1, TileID.LargePiles2, true, rand);
                                }
                            }
                            if (WorldGen.genRand.NextBool(2))
                ...
                tasks.Add(new PassLegacy("Hall of Heroes", delegate (GenerationProgress progress, GameConfiguration configuration)
                {
                    ...
                    while (!placed)
                    {
                        ...
                        if (tile.TileType != TileID.Stone || !GenVars.structures.CanPlace(new Rectangle((int)origin2.X, (int)origin2.Y, 84, 43)))
                            continue;
        ...
        public static void ElderWoodChest(int x, int y, int ID = 0)
        {
            ...
            if (PlacementSuccess >= 0)
            {
                ...
                if (ID == 2 || RedeHelper.GenChance(.25f))
                {
        ...
        public static void UndergroundPortalGen(ref GenerationProgress progress)
        {
            #region Underground Portal
            progress.Message = "Thinking with portals...Underground Portal";
            Dictionary<Color, int> colorToTile = new()
            {
                [new Color(255, 0, 0)] = TileType<GathicStoneBrickTile>(),
                [new Color(200, 0, 0)] = TileType<GathicGladestoneBrickTile>(),
                [new Color(0, 255, 0)] = TileType<GathicStoneTile>(),
                [new Color(0, 200, 0)] = TileType<GathicGladestoneTile>(),
                [new Color(0, 0, 255)] = TileType<AncientHallBrickTile>(),
                [new Color(100, 90, 70)] = TileType<AncientDirtTile>(),
                [new Color(200, 200, 50)] = TileType<AncientGoldCoinPileTile>(),
                [new Color(180, 180, 150)] = TileID.AmberGemspark,
                [new Color(150, 150, 150)] = -2,
                [Color.Black] = -1
            };
            ...
            Dictionary<Color, int> colorToWall = new()
            {
                [new Color(0, 0, 255)] = WallType<GathicStoneBrickWallTile>(),
                [new Color(0, 0, 200)] = WallType<GathicGladestoneBrickWallTile>(),
                [new Color(255, 0, 0)] = WallType<GathicStoneWallTile>(),
                [new Color(200, 0, 0)] = WallType<GathicGladestoneWallTile>(),
                [new Color(0, 255, 0)] = WallType<AncientHallPillarWallTile>(),
                [new Color(150, 150, 150)] = -2,
                [Color.Black] = -1
            };
            ...
            while (!placed)
            {
                ...
                WorldUtils.Gen(origin.ToPoint() - new Point(40, 40), new Shapes.Rectangle(98 + 80, 47 + 40), Actions.Chain(new GenAction[]
                {
                    new Actions.SetLiquid(0, 0)
                }));
                ...
                TexGen gen = TexGen.GetTexGenerator(tex, colorToTile, texWall, colorToWall, null, texSlope);
                gen.Generate((int)origin.X, (int)origin.Y, true, false, progress);


                gathicPortalVector = origin;
                placed = true;

                GenVars.structures.AddProtectedStructure(new Rectangle((int)gathicPortalVector.X, (int)gathicPortalVector.Y, 98, 47));
            }
            ...
            for (int i = originPoint.X; i < originPoint.X + 98; i++)
            {
                for (int j = originPoint.Y; j < originPoint.Y + 47; j++)
                {
                    switch (Framing.GetTileSafely(i, j).TileType)
                    {
                        case TileID.AmberGemspark:
                            Framing.GetTileSafely(i, j).ClearTile();
                            WorldGen.PlaceObject(i, j, (ushort)TileType<GraveSteelAlloyTile>(), true);
                            break;
                    }
                    if (WorldGen.genRand.NextBool(3))
                        WorldGen.PlacePot(i, j - 1);
        ...
        public static void JoShrineGen(ref GenerationProgress progress)
        {
            #region Jo Shrine
            ...
            while (!placed)
            {
                ...
                for (int i = 0; i <= dims.X; i++)
                {
                    ...
                    {
                        bool a = !GenVars.structures.CanPlace(new Rectangle((int)origin.X - 10, (int)origin.Y - 10, dims.X + 10, dims.Y + 10));
                        if (Main.remixWorld || Main.zenithWorld)
                            a = false;
                        int type = Framing.GetTileSafely((int)origin.X + i, (int)origin.Y + j).TileType;
                ...
                int attempts2 = 0;
                bool placed2 = false;
                while (!placed2 && attempts2++ < 200)
                {
                    int placeX2 = (int)origin.X - 55;
                    int placeY2 = WorldGen.genRand.Next(50, 80);
                    if (!WorldGen.InWorld(placeX2, placeY2))
                        continue;

                    bool whitelist2 = false;
                    for (int i = 0; i <= 144; i++)
                    {
                        for (int j = 0; j <= 80; j++)
                        {
                    ...
                    if (attempts2 >= 200)
                        break;
                    Point16 origin2 = new(placeX2, placeY2);

                    GatewayIsland biome = new();
                    GatewayIslandClear delete = new();
                    delete.Place(origin2.ToPoint(), GenVars.structures);
                    biome.Place(origin2.ToPoint(), GenVars.structures);

                    GoldenGatewayVector = origin2.ToVector2();
                    placed2 = true;
                }
                if (!placed2)
                    continue;

                GenUtils.PlaceStructure("WorldGeneration/JShrine", origin.ToPoint16());
    ...
    public class GenUtils
    {
        ...
        public static void ClearTrees(Point16 size, Point16 origin)
        {
            ClearTrees(size, origin.ToPoint());
        }
        public static void ClearTrees(Point16 size, Point origin)
        {
            for (int i = origin.X - 1; i < origin.X + size.X + 1; i++)
            {
                for (int j = origin.Y - 1; j < origin.Y + size.Y; j++)
                {
                    Tile tile = Framing.GetTileSafely(i, j);
                    if (tile.HasTile && tile.TileType is TileID.Trees or TileID.VanityTreeSakura or TileID.VanityTreeYellowWillow)
                        WorldGen.KillTile(i, j, noItem: true);
        ...
        public static void PlaceStructure(string path, Point16 origin, bool resetTileVisual = false)
        {
            Mod mod = Redemption.Instance;
            ...
            if (!resetTileVisual)
                return;
            for (int i = origin.X; i < origin.X + point.X; i++)
            {
                for (int j = origin.Y; j < origin.Y + point.Y; j++)
                    WorldGen.KillTile(i, j, true);
            }
        }
        public static void PlaceSkeletalRemains(int i, int j)
        {
            switch (WorldGen.genRand.Next(7))
            {
                default:
                    ObjectPlace(i, j - 1, ModContent.TileType<SkeletonRemainsTile1>(), 0, WorldGen.genRand.NextBool() ? -1 : 1);
                    break;
                case 1:
                    ObjectPlace(i, j - 1, ModContent.TileType<SkeletonRemainsTile2>(), 0, WorldGen.genRand.NextBool() ? -1 : 1);
                    break;
                case 2:
                    ObjectPlace(i, j - 1, ModContent.TileType<SkeletonRemainsTile3>(), 0, WorldGen.genRand.NextBool() ? -1 : 1);
                    break;
                case 3:
                    ObjectPlace(i, j - 1, ModContent.TileType<SkeletonRemainsTile4>(), 0, WorldGen.genRand.NextBool() ? -1 : 1);
                    break;
                case 4:
                    ObjectPlace(i, j - 1, ModContent.TileType<SkeletonRemainsTile5>(), 0, WorldGen.genRand.NextBool() ? -1 : 1);
                    break;
                case 5:
                    ObjectPlace(i, j - 1, ModContent.TileType<SkeletonRemainsTile6>(), 0, WorldGen.genRand.NextBool() ? -1 : 1);
                    break;
                case 6:
                    ObjectPlace(i, j - 1, ModContent.TileType<SkeletonRemainsTile7>(), 0, WorldGen.genRand.NextBool() ? -1 : 1);
                    break;
            }
        }
    }
