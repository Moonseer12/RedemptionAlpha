...
{
    public class CorruptedDAN_Proj : TrueMeleeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                if ((Projectile.localAI[1] == 0 && Projectile.localAI[0] > player.HeldItem.useTime * 4) || (Projectile.localAI[1] == 1 && Projectile.localAI[0] >= player.HeldItem.useTime * 6))
            ...
            Vector2 gunPos = Projectile.Center + RedeHelper.PolarVector(46 * Projectile.spriteDirection, Projectile.rotation) + RedeHelper.PolarVector(-4, Projectile.rotation + MathHelper.PiOver2);
            Vector2 gunSmokePos = Projectile.Center + RedeHelper.PolarVector(46 * Projectile.spriteDirection, Projectile.rotation) + RedeHelper.PolarVector(-8, Projectile.rotation + MathHelper.PiOver2);
            offset -= 6;
            rotOffset += 0.1f;
            int weaponDamage;
                ...
                if (player.PickAmmo(player.HeldItem, out rocket, out shootSpeed, out weaponDamage, out weaponKnockback, out usedAmmoId, !Main.rand.NextBool(3)))
                {
                    ...
                    if (Projectile.owner == Main.myPlayer)
                    {
                        for (int i = 0; i < shotNum; i++)
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), gunPos, RedeHelper.PolarVector(shootSpeed + Main.rand.Next(-4, 5), (Main.MouseWorld - gunPos).ToRotation() + Main.rand.NextFloat(-0.2f, 0.2f)), ProjectileType<DAN_Rocket>(), (int)(Projectile.damage * .75f), Projectile.knockBack, player.whoAmI);
                    }

                    for (int i = 0; i < 10; i++)
                    {
                        int num5 = Dust.NewDust(gunSmokePos, 8, 18, DustID.Smoke, Projectile.velocity.X / 2f, Projectile.velocity.Y / 2f, Scale: 2);
                    ...
                    for (int i = 0; i < 20; i++)
                    {
                        int num5 = Dust.NewDust(gunSmokePos, 8, 18, DustID.Wraith, Projectile.velocity.X / 2f, Projectile.velocity.Y / 2f, Scale: 2);
                        Main.dust[num5].velocity = RedeHelper.PolarVector(Main.rand.NextFloat(6, 8) * Projectile.spriteDirection, Projectile.rotation + Main.rand.NextFloat(-0.2f, 0.2f));
            ...
            {
                if (!player.channel && (Projectile.localAI[1] == 0 || Projectile.localAI[0] < player.HeldItem.useTime * 6 / player.GetAttackSpeed(DamageClass.Ranged)))
                {
                    Projectile.Kill();
                    return;
                }
                if (Projectile.localAI[1] == 0)
                {
                    ...
                    {
                        if (player.PickAmmo(player.HeldItem, out rocket, out shootSpeed, out weaponDamage, out weaponKnockback, out usedAmmoId, !Main.rand.NextBool(10)))
                        {
                            offset = 12;
                            ...
                            if (Projectile.owner == Main.myPlayer)
                                Projectile.NewProjectile(Projectile.GetSource_FromAI(), gunPos, RedeHelper.PolarVector(shootSpeed, Projectile.velocity.ToRotation()), ProjectileType<DAN_Rocket>(), (int)(Projectile.damage * .5f), Projectile.knockBack, player.whoAmI, 1);
                ...
                else
                {
                    ...
                    if (Projectile.localAI[0] == (int)(player.HeldItem.useTime * 6 / player.GetAttackSpeed(DamageClass.Ranged)))
                    {
                        ...
                        if (Projectile.owner == Main.myPlayer)
                        {
                            ...
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), gunPos, RedeHelper.PolarVector(3, Projectile.velocity.ToRotation()), ProjectileType<DAN_Laser>(), Projectile.damage * 6, Projectile.knockBack, player.whoAmI, Projectile.whoAmI, 1);
                        }
                        for (int i = 0; i < 15; i++)
                        {
