...
namespace Redemption.Items.Weapons.PreHM.Melee
{
    public class NoblesHalberd_SlashProj : TrueMeleeProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 5; i++)
                Dust.NewDustPerfect(target.Center + directionTo + new Vector2(0, 35) + player.velocity, DustType<DustSpark>(), directionTo.RotatedBy(Main.rand.NextFloat(-0.1f, 0.1f) - MathHelper.PiOver2 + (Projectile.spriteDirection == 1 ? MathHelper.Pi : 0)) * -Main.rand.NextFloat(4f, 5f) + (player.velocity / 2), 0, new Color(188, 188, 152) * 0.8f, 3f);
